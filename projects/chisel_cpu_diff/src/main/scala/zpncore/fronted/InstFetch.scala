...
//  val pc = RegInit("h8000_0000".U(WLEN.W))            //* nextPC = 0x8000_0000,可以取到正确指令
  ...
  io.imem.inst_valid := !io.stall                        //* IF valid暂停无效，请求AXI传输指令
...
// 握手成功，从总线上取到指令，更新寄存器PC与inst
  ...
  val ifPC = Mux(fire, 
              Mux(io.pcSrc =/= 0.U, io.nextPC, pc + 4.U),
               pc)
  pc := ifPC                                          //* 更新pc/inst寄存器值,并保持当前寄存器状态 
  inst := ifInst

  io.imem.inst_req := REQ_READ
  io.imem.inst_addr := ifPC.asUInt()
...
/*
  ...
  io.imem.inst_valid := !io.stall//true.B                        // IF valid暂停无效，请求AXI传输指令
  val fire = io.imem.inst_valid && io.imem.inst_ready            // 握手成功，从总线上取出指令
  ...
  val ifPC = Mux(IFDone,
            Mux(io.pcSrc === 0.U, 
              Mux(io.stall, pc, pc + 4.U),
                io.nextPC),
                  pc)
  IFDone := RegNext(fire)                                      // PC改变需要打一拍，才能获得当前inst
  pc := ifPC                                          // 更新pc/inst寄存器值,并保持当前寄存器状态 
  inst := ifInst
